...
/// Performance-oriented library screen.
///
/// The library does not need a live database subscription for every visible
/// row. It loads a small snapshot, keeps filtering/search in memory, and
/// refreshes explicitly after mutations or returning from the reader. This
/// avoids several nested Drift StreamBuilders rebuilding the entire shelf.
class OptimizedLibraryScreen extends ConsumerStatefulWidget {
  const OptimizedLibraryScreen({super.key});

  @override
  ConsumerState<OptimizedLibraryScreen> createState() =>
      _OptimizedLibraryScreenState();
}

class _OptimizedLibraryScreenState
    extends ConsumerState<OptimizedLibraryScreen> {
  ...
  Future<void> _loadSnapshot({bool showLoading = false}) async {
    ...
    try {
      final results = await Future.wait<dynamic>([
        ...
        db.watchAllReadingSessions().first,
        db.watchShelves().first,
        db.watchTags().first,
        db.watchFolders().first,
      ]);

      final sessions = results[3] as List<ReadingSessionRow>;
      ...
      setState(() {
        ...
        _seconds = seconds;
        _shelves = results[4] as List<ShelfRow>;
        _tags = results[5] as List<TagRow>;
        _folders = results[6] as List<LibraryFolderRow>;
        _loading = false;
      });
    } catch (e) {
      if (!mounted) return;
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: _loading
          ...
          : RefreshIndicator(
              onRefresh: () => _loadSnapshot(showLoading: false),
              ...
              if (_shelfId != null || _tagId != null)
                TextButton(
                  onPressed: () async {
                    ...
                    await _loadSnapshot(showLoading: true);
                  },
                  child: const Text('해제'),
                ),
            ],
  ...
  Widget _chip(String text, bool selected, VoidCallback tap) {
    return FilterChip(label: Text(text), selected: selected, onSelected: (_) => tap());
  }
    ...
    return GestureDetector(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(book.author ?? '작가 미상', maxLines: 1, overflow: TextOverflow.ellipsis, style: Theme.of(context).textTheme.bodySmall),
          const SizedBox(height: 3),
          Text(_status(progress, state, _seconds[book.id] ?? 0), maxLines: 1, overflow: TextOverflow.ellipsis, style: Theme.of(context).textTheme.labelSmall),
        ],
      ),
    );
  }
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  ...
                  Text(_status(progress, state, _seconds[book.id] ?? 0), style: Theme.of(context).textTheme.labelSmall),
                ],
              ),
            ),
            ...
            Padding(
              ...
              child: Row(
                children: [
                  ...
                  const Spacer(),
                  TextButton.icon(onPressed: () { Navigator.pop(sheetContext); _addFolder(); }, icon: const Icon(Icons.add), label: const Text('추가')),
                ],
              ),
            ),
      ...
      builder: (sheetContext) => SafeArea(
        child: ListView(
          ...
          children: [
            ...
            ListTile(leading: const Icon(Icons.menu_book_outlined), title: const Text('계속 읽기'), onTap: () { Navigator.pop(sheetContext); _open(book.originalUri); }),
            ListTile(leading: const Icon(Icons.delete_outline), title: const Text('서재에서 제거'), onTap: () async {
              Navigator.pop(sheetContext);
